        ...
        <nav className="flex justify-between pt-[32px] items-center">
            ...
            <div className="pl-3  hidden md:flex">
                <div className="flex text-2xl not-italic font-bold">
                    <Anchor to="/">
                        <button className="pr-4 pt-3"> {`${home}`}</button>
                    </Anchor>
                    <Anchor to="/nav/cities">
                        <button className="pr-4 pt-3">{`${cities}`}</button>
                    </Anchor>
                    {isLoggedIn ? (
                        <button
                            onClick={handleLogout}
                            className="flex flex-row text-2xl py-3 px-8 leading-normal items-center hover:bg-indigo-500 bg-indigo-700 text-white rounded-[10px] gap-2.5"
                        >
                            Logout
                        </button>
                    ) : (
                        <Anchor to="/nav/auth/login">
                            <button className="flex flex-row text-2xl py-3 px-8 leading-normal items-center hover:bg-indigo-500 bg-indigo-700 text-white rounded-[10px] gap-2.5">
